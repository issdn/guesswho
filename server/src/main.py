...
async def game(token: str, websocket: WebSocket)  -> None:
...
async def game_data(token: str) -> list[tuple[str, str]]:
    """
    List of names of characters generated by PlayersManager.get_random_image_names()
    """
    return phase_queues[token].players_manager.image_name_path_tuples_list
...
async def image(token: str, image: str) -> (FileResponse | HTTPException):
    ...
    """
    return FileResponse(abspath(Config.CHARACTER_IMAGES_DIRECTORY_PATH + image))
